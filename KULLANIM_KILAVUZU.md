...
**Daha kolay bir yol — sürükleyip bırakma:** Bir Excel dosyasını (`.xlsx`) bilgisayarınızdaki klasörden alıp doğrudan programın penceresinin üzerine **sürükleyip bırakabilirsiniz**. Henüz hiçbir dosya açık değilken bunu yaptığınızda dosya, tıpkı **Excel Aç** ile açmışsınız gibi doğrudan yüklenir; ekstra bir soru sorulmaz. (Zaten bir dosya açıkken sürükleyip bırakırsanız küçük bir seçim penceresi çıkar; bunu [Birleştirme](#birleştirme) bölümünde anlatıyoruz.)
...
### Sürükleyip bırakarak birleştirme

Bir dosya zaten açıkken, birleştirmek istediğiniz ikinci Excel dosyasını doğrudan pencerenin üzerine **sürükleyip bırakabilirsiniz**. Bu durumda küçük bir seçim penceresi çıkar ve size iki seçenek sunar:

- **Aç** — Açık olan dosyanız kapanır ve sürüklediğiniz dosya onun yerine açılır (birleştirme yapılmaz).
- **Birleştir** — Sürüklediğiniz dosya ikinci dosya (B) olarak alınır ve yukarıda anlatılan normal birleştirme akışı başlar (sütunlar çakışıyorsa seçim ekranı yine gösterilir).

Pencereyi **İptal** ile kapatırsanız hiçbir şey olmaz; ne açık dosyanız değişir ne de birleştirme başlar.

![Sürüklenen dosya bırakıldığında çıkan Aç / Birleştir / İptal seçim penceresi](screenshots/09_surukle_birak_secim.png)
...
**S: Dosyayı sürükleyip bıraktım ama hiçbir şey olmadı.**
C: Program yalnızca **tek** bir Excel dosyasını (`.xlsx` uzantılı) sürükleyip bırakmanıza izin verir. Aynı anda birden fazla dosya sürüklerseniz veya Excel olmayan bir dosya (örneğin bir resim ya da Word belgesi) bırakırsanız, program bunu görmezden gelir; bir hata da vermez. Tek bir `.xlsx` dosyası sürüklediğinizden emin olun.
